...
function OperatorList({ entries, color }: { entries: { name: string; count: number; lastDate: string }[]; color: string }) {
  return (
        ...
        <div key={name} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              background: color === 'var(--blue)' ? 'var(--blue-bg)' : 'var(--purple-bg)',
              border: `1px solid ${color === 'var(--blue)' ? 'var(--blue-border)' : 'var(--purple-border)'}`,
    ...
    <div className="page-content" style={{ paddingTop: 0 }}>
      ...
      {getOperatorStats(history).length > 0 && (
        <>
          ...
          <OperatorList entries={getOperatorStats(history)} color="var(--blue)" />
        </>
      )}
  ...
  return (
    <div className="page-content" style={{ paddingTop: 0 }}>
      ...
      {getOperatorStats(history).length > 0 && (
        <>
          <div className="section-title" style={{ marginBottom: 12 }}>Nach Mitarbeiter</div>
          <OperatorList entries={getOperatorStats(history)} color="var(--purple)" />
        </>
      )}
...
export default function StatistikScreen() {
  ...
  return (
    <>
      ...
      {tab === 'retouren' ? <RetourenTab isErik={isErik} /> : <VersandTab isErik={isErik} />}
    </>
  )
}
