...
import type { VersandEntry } from '@/lib/versandHistory'
...
export function VersandTabContent() {
  const [history, setHistory] = useState<VersandEntry[] | null>(null)
  const [configured, setConfigured] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    let cancelled = false
    apiGet<{ entries: VersandEntry[]; configured: boolean }>('/api/versand-history')
      .then((data) => {
        if (cancelled) return
